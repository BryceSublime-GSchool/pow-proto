"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var router_1 = require('@angular/router');
var MountainDetailComponent = (function () {
    function MountainDetailComponent(router) {
        this.router = router;
    }
    MountainDetailComponent = __decorate([
        core_1.Component({
            selector: 'mountain-detail',
            template: "\n    <div class=\"content-box\">\n    </div>\n    <nav>\n      <a [routerLink]=\"['/']\" routerLinkActive=\"active\">\n        <img src=\"./app/assets/boreal.png\">\n      </a>\n      <a [routerLink]=\"['/the-mountain']\" routerLinkActive=\"active\">The Mountain</a>\n      <a [routerLink]=\"['/the-trip']\" routerLinkActive=\"active\">The Trip</a>\n      <a [routerLink]=\"['/things-to-do']\" routerLinkActive=\"active\">Things to Do</a>\n    </nav>\n    <router-outlet></router-outlet>\n  ",
            directives: [router_1.ROUTER_DIRECTIVES]
        }), 
        __metadata('design:paramtypes', [Object])
    ], MountainDetailComponent);
    return MountainDetailComponent;
}());
exports.MountainDetailComponent = MountainDetailComponent;
//# sourceMappingURL=mountain-detail.component.js.map